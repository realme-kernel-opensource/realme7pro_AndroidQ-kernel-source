...
/* add begin by zhangchaofan@ODM_LQ@Multimedia.TP, add tp cs gpio 2020-02-20*/
static int cs_gpio;
/* add end by zhangchaofan@ODM_LQ@Multimedia.TP, add tp cs gpio 2020-02-20 */
#endif /*ODM_LQ_EDIT*/
...
#ifdef ODM_LQ_EDIT
/* add begin by zhangchaofan@ODM_LQ@BSP.TP, TP CS 2020-02-20 */
/*
Interface for tp to control cs pin
*/
...
/* add end by zhangchaofan@ODM_LQ@BSP.TP, TP CS 2020-02-20 */
#endif /*ODM_LQ_EDIT*/
...
static void init_parse_dts(struct device *dev, struct touchpanel_data *ts)
{
    ...
    TPD_INFO("cs_gpio = %d\n", cs_gpio);
    if (gpio_is_valid(cs_gpio)) {
       rc = gpio_request(cs_gpio, "cs-gpio");
       if (rc)
           TPD_INFO("unable to request gpio [%d]\n", cs_gpio);
    } else {
...
/* modify begin by zhangchaofan@ODM_LQ@Multimedia.TP, remove invalid dts 2019-12-09 */
    ...
    /* modify end by zhaifeibiao@ODM_LQ@Multimedia.TP, for tp compatible 2020-06-14 */
	...
	} else if (g_tp_dev_vendor == TP_GOODIX){   //goodix==16 36
		rc = of_property_read_u32_array(np, "touchpanel,tx-rx-num", tx_rx_num, 2);
	} else if (g_tp_dev_vendor == TP_FOCAL){   //focal==16 36
		rc = of_property_read_u32_array(np, "touchpanel,tx-rx-num", tx_rx_num, 2);
        }
	if (rc) {
...
#ifdef ODM_TARGET_DEVICE_206B1
    /* modify end by zhaifeibiao@ODM_LQ@Multimedia.TP, for tp compatible 2020-06-16 */
...
EXIT:
    ...
    mutex_unlock(&ts->mutex);
#ifdef ODM_LQ_EDIT
	/* modify begin by zhangchaofan@ODM_LQ@Multimedia.TP, for tp esd 2020-02-20 */
	if (tp_gesture_esd_flag)
		msleep(250);
	/* modify end by zhangchaofan@ODM_LQ@Multimedia.TP, for tp esd 2020-02-20 */
#endif /*ODM_LQ_EDIT*/
NO_NEED_SUSPEND:
    complete(&ts->pm_complete);
