...
static int32_t Write_Partition(struct chip_data_nt36672c *chip_info, const u8 *fwdata, size_t fwsize)
{
    ...
    for (list = 0; list < chip_info->partition; list++) {
	    ...
	    /*add for TP download FW falied when fingerprint unlock the device by qujiong 20200311 begin*/
            //CTP_SPI_WRITE(chip_info->s_client, chip_info->fwbuf, len + 1);
            CTP_SPI_WRITE_FIRMWARE(chip_info->s_client, chip_info->fwbuf, len + 1);
            /*add for TP download FW falied when fingerprint unlock the device by qujiong 20200311 end*/
#if NVT_DUMP_SRAM
...
#ifdef ODM_LQ_EDIT
/* add begin by zhangchaofan@ODM_LQ@Multimedia.TP, for tp shutdown 2020-02-20*/
...
/* add end by zhangchaofan@ODM_LQ@Multimedia.TP, for tp shutdown 2020-02-20 */
#endif /*ODM_LQ_EDIT*/
...
static struct spi_driver tp_spi_driver = {
    ...
    .shutdown   = nvt_tp_shutdown, ///* modify by zhangchaofan@ODM_LQ@Multimedia.TP, for tp shutdown 2020-02-20 */
    .id_table   = tp_id,
    .driver = {
        .name   = TPD_DEVICE,
        .owner  = THIS_MODULE,
