...
bool __init tp_judge_ic_match(char *tp_ic_name)
{
    ...
    switch(get_project()) {
		...
		case 0x206B1:
			/* modify end by zhaifeibiao@ODM_LQ@Multimedia.TP, for tp compatible 2020-05-04 */
			if(strstr(tp_ic_name, "focaltech,fts") && (strstr(saved_command_line, "dsi_s6e8fc1x01_samsung_amoled_video_display"))){
				g_tp_dev_vendor = TP_FOCAL;
				tp_used_index = ft_3518;
				return true;
			}        
			if(strstr(tp_ic_name, "Goodix-gt9886") && (strstr(saved_command_line, "dsi_ams643xf01_samsung_amoled_cmd_display"))){
				g_tp_dev_vendor = TP_GOODIX;
				tp_used_index = gt_9886;
				return true;
			}
			/* modify end by zhaifeibiao@ODM_LQ@Multimedia.TP, for tp compatible 2020-05-04 */
			#endif /*ODM_LQ_EDIT*/
...
int tp_util_get_vendor(struct hw_resource *hw_res, struct panel_info *panel_data)
{
   ...
   panel_data->tp_type = g_tp_dev_vendor;
	...
	#endif /*ODM_LQ_EDIT*/
	} else if (tp_used_index == ft_3518){
		memcpy(panel_data->manufacture_info.version, "FOCAL", 5);
		//panel_data->firmware_headfile.firmware_data = FW_18621_HX83112A_NF_DSJM;
		//panel_data->firmware_headfile.firmware_size = sizeof(FW_18621_HX83112A_NF_DSJM);
	} else if (tp_used_index == gt_9886){
		memcpy(panel_data->manufacture_info.version, "GOODIX", 6);
		//panel_data->firmware_headfile.firmware_data = FW_18621_HX83112A_NF_DSJM;
		//panel_data->firmware_headfile.firmware_size = sizeof(FW_18621_HX83112A_NF_DSJM);
	}
