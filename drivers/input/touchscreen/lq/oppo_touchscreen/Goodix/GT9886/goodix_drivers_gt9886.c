...
void goodix_reset_via_gpio(struct chip_data_gt9886 *chip_info)
{
    ...
    if (gpio_is_valid(chip_info->hw_res->reset_gpio)) {
        ...
        udelay(2000);
		value = gpio_get_value(chip_info->hw_res->reset_gpio);
...
#if 0
static void goodix_tp_pinctrl_init(struct touchpanel_data *ts)
{
    int retval = 0;
    struct pinctrl *ts_pinctrl = NULL;
    struct pinctrl_state *pinctrl_state_active;

    ts_pinctrl = devm_pinctrl_get(ts->dev->parent);
    if (IS_ERR_OR_NULL(ts_pinctrl)) {
        retval = PTR_ERR(ts_pinctrl);
        dev_info(ts->dev->parent, "Target does not use pinctrl %d\n", retval);
	}
    ...
    if (IS_ERR_OR_NULL(pinctrl_state_active)) {
        ...
        dev_info(ts->dev->parent, "Can not lookup pmx_ts_active pinstate %d\n", retval);
    }

    retval = pinctrl_select_state(ts_pinctrl, pinctrl_state_active);
    if (retval < 0) {
        dev_info(ts->dev->parent, "Failed to select pmx_ts_actives pinstate %d\n", retval);
    }
}
#endif
