...
int touch_i2c_read(struct i2c_client *client, char *writebuf, int writelen, char *readbuf, int readlen)
{
    ...
    static unsigned char *wbuf = NULL;
    ...
    if ((writelen > 256) || (readlen > 256)) {
        TPD_INFO("%s: writelen(%d),readlen(%d) fail\n", __func__, writelen, readlen);
        mutex_unlock(&i2c_mutex);
        return -1;
    }

    if (!wbuf || !rbuf) {
        wbuf = kzalloc(FIX_I2C_LENGTH, GFP_KERNEL | GFP_DMA);
        rbuf = kzalloc(FIX_I2C_LENGTH, GFP_KERNEL | GFP_DMA);
        if (!wbuf || !rbuf) {
           TPD_INFO("%s: wbuf/rbuf kzalloc fail\n", __func__);
            mutex_unlock(&i2c_mutex);
            return -1; 
        }
    }
...
/*add for TP download FW falied when fingerprint unlock the device by qujiong 20200311*/
