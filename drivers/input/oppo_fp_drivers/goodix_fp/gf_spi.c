...
#endif
{
    ...
    if((FP_GOODIX_3268 != get_fpsensor_type())
		...
		&& (FP_GOODIX_OPTICAL_G3 != get_fpsensor_type())){
...
/*yangcaoping@ODM_LQ@BSP.fingerprint,2020/05/01 for fingerprint*/
    ...
    if (status == -1) {
        pr_err("msm_drm_register_client failed\n");
        return status;
    }
...
#ifdef ODM_TARGET_DEVICE_206B1
/*yangcaoping@ODM_LQ@BSP.fingerprint,2020/05/28 for fingerprint*/
    if (MSM_BOOT_MODE__FACTORY == get_boot_mode()) {
	    pr_err("in factory mode, and power off\n");
	    gf_power_on(gf_dev);
	    gf_power_off(gf_dev);
    }
#endif /*ODM_TARGET_DEVICE_206B1*/
