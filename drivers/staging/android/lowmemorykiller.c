 ...
 *totalram_pages 1943597 =>7500M, TotalMem 6700
 *filepage + freepage ~ 1G, set almk_totalram_ratio 8
 */
...
#define ALMK_MEM_INFO_TABLE_18621_NUM 4
static struct almk_mem_info almk_mem_info_table_18621[ALMK_MEM_INFO_TABLE_18621_NUM] = {
	{
		.pages_swap		= ALMK_MB_to_PAGES(1000),
		.pages_anon		= ALMK_MB_to_PAGES(20),
		.pages_file		= ALMK_MB_to_PAGES(2000),
		.adj		= 404,
	},
	{
		.pages_swap		= ALMK_MB_to_PAGES(1950),
		.pages_anon		= ALMK_MB_to_PAGES(20),
		.pages_file		= ALMK_MB_to_PAGES(2500),
		.adj		= 300,
	},
	{
		.pages_swap		= ALMK_MB_to_PAGES(2100),
		.pages_anon		= ALMK_MB_to_PAGES(20),
		.pages_file		= ALMK_MB_to_PAGES(2300),
		.adj		= 200,
	},
	{
		.pages_swap		= ALMK_MB_to_PAGES(2300),
		.pages_anon		= ALMK_MB_to_PAGES(20),
		.pages_file		= ALMK_MB_to_PAGES(2000),
		.adj		= 100,
	}
...
#define ALMK_MEM_INFO_TABLE_DEFAULT_NUM 4
static struct almk_mem_info almk_mem_info_table_default[ALMK_MEM_INFO_TABLE_DEFAULT_NUM] = {
	{
		.pages_swap		= ALMK_MB_to_PAGES(1950),
		.pages_anon		= ALMK_MB_to_PAGES(650),
		.pages_file		= ALMK_MB_to_PAGES(300),
		.adj		= 404,
	},
	{
		.pages_swap		= ALMK_MB_to_PAGES(2300),
		.pages_anon		= ALMK_MB_to_PAGES(700),
		.pages_file		= ALMK_MB_to_PAGES(150),
		.adj		= 300,
	},
	{
		.pages_swap		= ALMK_MB_to_PAGES(2400),
		.pages_anon		= ALMK_MB_to_PAGES(750),
		.pages_file		= ALMK_MB_to_PAGES(120),
		.adj		= 200,
	},
	{
		.pages_swap		= ALMK_MB_to_PAGES(2500),
		.pages_anon		= ALMK_MB_to_PAGES(750),
		.pages_file		= ALMK_MB_to_PAGES(120),
		.adj		= 100,
	}
...
static unsigned int almk_debug_enable = 0;
module_param_named(almk_debug_enable, almk_debug_enable, uint, S_IRUGO | S_IWUSR);

#define almk_dbg(fmt, ...) \
	if (almk_debug_enable) printk(KERN_INFO pr_fmt(fmt), ##__VA_ARGS__)


#include <soc/oppo/oppo_project.h>
static int oppo_adjust_minadj(short *min_score_adj)
{
	...
	struct almk_mem_info *almk_mem_info_p;
	int almk_mem_info_table_num;
	...
	if (totalram_pages <= (ALMK_NR_PAGES_1GB*4)) {
		...
		almk_dbg("oppo_adjust_minadj:project=%u\n", project);

		if(0) {
			almk_mem_info_p = almk_mem_info_table_18621;
			almk_mem_info_table_num = ALMK_MEM_INFO_TABLE_18621_NUM;
			almk_dbg("almk_mem_info_table_18621,size:%d\n", almk_mem_info_table_num);
		} else {
			almk_mem_info_p = almk_mem_info_table_default;
			almk_mem_info_table_num = ALMK_MEM_INFO_TABLE_DEFAULT_NUM;
			almk_dbg("almk_mem_info_table_default,size:%d\n", almk_mem_info_table_num);
		}

		for (i = 0; i < almk_mem_info_table_num; i++) {

			almk_dbg("almk_mem_info_table:%d, swap=%ld, anon=%lu, file=%lu,adj:%d \n",
				i,
				almk_mem_info_p->pages_swap, almk_mem_info_p->pages_anon,
				almk_mem_info_p->pages_file, almk_mem_info_p->adj);

			if ((*min_score_adj > almk_mem_info_p->adj) &&
				(almk_pages_swap > almk_mem_info_p->pages_swap) &&
				(almk_pages_anon > almk_mem_info_p->pages_anon) &&
				(almk_pages_file < almk_mem_info_p->pages_file)) {

					almk_adjust_minadj_level = i+1;
					almk_dbg("oppo_adjust_minadj:almk_pages_swap=%ld, almk_pages_anon=%lu, almk_pages_file=%lu,level:%d \n",
						almk_pages_swap, almk_pages_anon, almk_pages_file, almk_adjust_minadj_level);

					*min_score_adj = almk_mem_info_p->adj;
			}
...
/*tangshaoqing@RM.BSP.Kernel.MM 2019-03-28 modify for adaptive lowmemkill adj */
	if (atomic_read(&shift_adj)) {
		oppo_adjust_minadj(min_score_adj);
	}
#endif /*VENDOR_EDIT*/
...
#ifdef VENDOR_EDIT
/*tangshaoqing@RM.BSP..kernel.mm, 2019-03-16, modify almk_totalram_ratio*/
...
#ifdef VENDOR_EDIT
	/*tangshaoqing@RM.BSP..kernel.mm, 2019-03-16, modify almk_totalram_ratio*/
	if (totalram_pages <= TOTALRAM_PAGES_2GB)
		almk_totalram_ratio = 2;
	else if (totalram_pages <= TOTALRAM_PAGES_3GB)
		almk_totalram_ratio = 3;
	else if (totalram_pages <= TOTALRAM_PAGES_4GB)
		almk_totalram_ratio = 1;
#endif /* VENDOR_EDIT */
		...
		size_t count,loff_t *off)
{
	...
	len = snprintf(&page[len], 256, "adaptive_lowmem_kill_count:%lu\n"
