	...
	unsigned int cmd, void *arg)
{
	...
	case VIDIOC_CAM_FTM_POWNER_UP:
		...
		if (s_ctrl->sensordata->slave_info.sensor_id == 0x586) {
		...
		} else if(s_ctrl->sensordata->slave_info.sensor_id == 0x0682) {
			/*Man@Li.RM.Camera, 20200511, add this "else if" for imx682 test*/
			...
			CAM_ERR(CAM_SENSOR,"FTM GET imx682 setting");
			...
			/*Man@Li.RM.Camera, 20200511, add this "else if" for ov02b1b test*/
			sensor_setting.reg_setting = sensor_settings.ov02b1b_setting.reg_setting;
			sensor_setting.addr_type = CAMERA_SENSOR_I2C_TYPE_BYTE;
			sensor_setting.data_type = CAMERA_SENSOR_I2C_TYPE_BYTE;
			sensor_setting.size = sensor_settings.ov02b1b_setting.size;
			sensor_setting.delay = sensor_settings.ov02b1b_setting.delay;
		...
		} else if(s_ctrl->sensordata->slave_info.sensor_id == 0x9502) {
			/*Man@Li.RM.Camera, 20200511, add this "else if" for gc02k0 test*/
			sensor_setting.reg_setting = sensor_settings.gc02k0_setting.reg_setting;
			sensor_setting.addr_type = CAMERA_SENSOR_I2C_TYPE_BYTE;
			sensor_setting.data_type = CAMERA_SENSOR_I2C_TYPE_BYTE;
			sensor_setting.size = sensor_settings.gc02k0_setting.size;
			sensor_setting.delay = sensor_settings.gc02k0_setting.delay;
		} else if(s_ctrl->sensordata->slave_info.sensor_id == 0x3241) {
			/*Man@Li.RM.Camera, 20200511, add this "else if" for ov32a1q test*/
