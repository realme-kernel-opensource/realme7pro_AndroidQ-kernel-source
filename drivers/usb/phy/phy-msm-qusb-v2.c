...
static void qusb_phy_get_tune1_param(struct qusb_phy *qphy)
{
	...
	pr_debug("%s(): reg:%d efuse based tune1 value:%d\n",
				__func__, reg, qphy->tune_val);
	#ifdef ODM_LQ_EDIT
	/*xiaoyong.nan@ODM_LQ@BSP.OTG,2020/07/27,Based on case04687894 to modify the OTG problem */
		reg = reg & 0x0f;
		reg |= (qphy->tune_val << 4);
	#else
	if (qphy->tune_val) {
