...
static int bq27541_get_battery_soc(void)
{
	...
	if (oppo_vooc_get_allow_reading() == true) {
		...
		if ((gauge_ic->batt_vol_pre < 3500) && (soc >= 100)) {
			pr_err(" batt_vol_pre = %d soc = %d \n",gauge_ic->batt_vol_pre, soc);
			ret = bq27541_read_i2c(gauge_ic->cmd_addr.reg_soc, &soc);
			if (ret) {
				dev_err(gauge_ic->dev, "error reading soc.ret:%d\n", ret);
				goto read_soc_err;
...
/*
static char* convertUnCharToStr(char* str, unsigned char* UnChar, int ucLen)
{
	int i = 0;
	for(i = 0; i < ucLen; i++)
	{
...
int dump_gauge_registers(bool allow_reading){
	int ret = 0, temp = 0, volt = 0, cur = 0, rm = 0, fcc = 0, soc = 0;
	u8 value_71[32];
	u8 value_73[24];
	u8 value_74[24];
	u8 value_75[20];
	...
	if (atomic_read(&gauge_ic->suspended) == 1) {
		printk(KERN_ERR "dump_gauge_registers: suspend, ignore.\n");
		return 0;
	}

	if ((!oppo_vooc_get_fastchg_started() && !allow_reading) || (oppo_vooc_get_fastchg_started() && allow_reading)) {
		memset(value_71, 0, 32);
		memset(value_73, 0, 24);
		memset(value_74, 0, 24);
		memset(value_75, 0, 20);

		ret = bq27541_read_i2c(gauge_ic->cmd_addr.reg_temp, &temp);
		if (ret) {
			printk(KERN_ERR "dump_gauge_registers: error reading temperature\n");
			temp = -400;
		}

		ret = bq27541_read_i2c(gauge_ic->cmd_addr.reg_volt, &volt);
		if (ret) {
			printk(KERN_ERR "dump_gauge_registers: error reading volt\n");
			volt = 0;
		}

		ret = bq27541_read_i2c(gauge_ic->cmd_addr.reg_nac, &cur);
		if (ret) {
			printk(KERN_ERR "dump_gauge_registers: error reading current\n");
			cur = 0;
		}

		ret = bq27541_read_i2c(gauge_ic->cmd_addr.reg_rm, &rm);
		if (ret) {
			printk(KERN_ERR "dump_gauge_registers: error reading rm\n");
			rm = 0;
		}

		ret = bq27541_read_i2c(gauge_ic->cmd_addr.reg_fcc, &fcc);
		if (ret) {
			printk(KERN_ERR "dump_gauge_registers: error reading fcc\n");
			fcc = 0;
		}

		ret = bq27541_read_i2c(gauge_ic->cmd_addr.reg_soc, &soc);
		if (ret) {
			printk(KERN_ERR "dump_gauge_registers: error reading soc\n");
			soc = 0;
		}

		mutex_lock(&bq28z610_alt_manufacturer_access);
		bq27541_i2c_txsubcmd(0x3E, 0x0071);
		usleep_range(1000, 1000);
		bq27541_read_i2c_block(0x40, 32, value_71);

		usleep_range(1000, 1000);
		bq27541_i2c_txsubcmd(0x3E, 0x0073);
		usleep_range(1000, 1000);
		bq27541_read_i2c_block(0x40, 24, value_73);

		usleep_range(1000, 1000);
		bq27541_i2c_txsubcmd(0x3E, 0x0074);
		usleep_range(1000, 1000);
		bq27541_read_i2c_block(0x40, 24, value_74);

		usleep_range(1000, 1000);
		bq27541_i2c_txsubcmd(0x3E, 0x0075);
		usleep_range(1000, 1000);
		bq27541_read_i2c_block(0x40, 20, value_75);
		mutex_unlock(&bq28z610_alt_manufacturer_access);

		printk(KERN_ERR "dump_gauge_registers: temp: %d, volt: %d, current: %d, rm: %d, fcc: %d, soc: %d\n",
			temp, volt, cur, rm, fcc, soc);

		printk(KERN_ERR "dump_gauge_registers: value_71: %s, cell_volt1: %d, cell_volt2: %d\n",
			convertUnCharToStr(output, value_71, 32), ((value_71[1] << 8) | value_71[0]), ((value_71[3] << 8) | value_71[2]));

		printk(KERN_ERR "dump_gauge_registers: value_73: %s, True_Rem_Q: %d, Init_Q: %d, TrueFullChgQ: %d, T_sim: %d\n",
			convertUnCharToStr(output, value_73, 24), ((value_73[1] << 8) | value_73[0]), ((value_73[3] << 8) | value_73[2]),
			((value_73[1] << 9) | value_73[8]), ((value_73[13] << 8) | value_73[12]));

		printk(KERN_ERR "dump_gauge_registers: value_74: %s, DOD0_1: %d, DOD0_2: %d, DOD0PassedQ: %d, DOD0Time: %d, DODEOC_1: %d, DODEOC_2: %d\n",
			convertUnCharToStr(output, value_74, 24), ((value_74[11] << 8) | value_74[10]), ((value_74[13] << 8) | value_74[12]),
			((value_74[15] << 8) | value_74[14]), ((value_74[19] << 8) | value_74[18]),
			((value_74[21] << 8) | value_74[20]), ((value_74[23] << 8) | value_74[22]));

		printk(KERN_ERR "dump_gauge_registers: value_75: %s, QMAX1: %d, QMAX2: %d\n",
			convertUnCharToStr(output, value_75, 20), ((value_75[1] << 8) | value_75[0]), ((value_75[3] << 8) | value_75[2]));
	}
	return 0;
}
*/
