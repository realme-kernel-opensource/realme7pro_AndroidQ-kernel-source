...
static int mp2650_debug = 0;
module_param(mp2650_debug, int, 0644);
#define OPEN_LOG_BIT BIT(0)
MODULE_PARM_DESC(mp2650_debug, "debug mp2650");

#define mp2650_dbg(fmt, ...)			\
	do {							\
		if (OPEN_LOG_BIT & mp2650_debug)		\
			chg_err(fmt, ##__VA_ARGS__);	\
	} while (0)
...
static int mp2650_config_interface(int RegNum, int val, int MASK)
{
    ...
    mp2650_dbg(" Reg[%x]=0x%x %d\n", RegNum, mp2650_reg, MASK);

    mp2650_reg &= ~MASK;
    mp2650_reg |= val;

    mp2650_dbg(" write Reg[%x]=0x%x %d\n", RegNum, mp2650_reg, val);
...
int mp2650_input_current_limit_ctrl_by_vooc_write(int current_ma)
{
	...
	for (count=(tmp / 500); count > 0; count--) {
		mp2650_dbg("set charge current limit = %d\n", 500*count);
	...
	for (count=1; count <= (current_ma /500); count++) {
		mp2650_dbg("set charge current limit1 = %d\n", 500*count);
		tmp = 500*count - REG00_MP2650_1ST_CURRENT_LIMIT_OFFSET;
		tmp = tmp / REG00_MP2650_1ST_CURRENT_LIMIT_STEP;
		rc = mp2650_config_interface(REG00_MP2650_ADDRESS, tmp << REG00_MP2650_1ST_CURRENT_LIMIT_SHIFT, REG00_MP2650_1ST_CURRENT_LIMIT_MASK);
		rc = mp2650_config_interface(REG0F_MP2650_ADDRESS, tmp << REG0F_MP2650_2ND_CURRENT_LIMIT_SHIFT, REG0F_MP2650_2ND_CURRENT_LIMIT_MASK);
...
void mp2650_dump_registers(void)
{
    ...
    struct chip_mp2650 *chip = charger_ic;
    unsigned int val_buf[MP2650_REG_NUMBER] = {0x0};
    ...
    if(dump_count == DUMP_REG_LOG_CNT_30S) {
        dump_count = 0;
        for (addr = MP2650_FIRST_REG; addr <= MP2650_LAST_REG; addr++) {
            rc = mp2650_read_reg(addr, &val_buf[addr]);
            if (rc) {
                chg_err("Couldn't read 0x%02x rc = %d\n", addr, rc);
            }
        }
        rc = mp2650_read_reg(0x48, &val_buf[MP2650_LAST_REG + 1]);
        if (rc) {
             chg_err("Couldn't  read 0x49 rc = %d\n", rc);
        }
    #ifdef OPPO_OPEN_ADBROOT_VERSION
        rc = mp2650_read_reg(0x2B, &val_buf[MP2650_LAST_REG + 2]);
        if (rc) {
             chg_err("Couldn't  read 0x48 rc = %d\n", rc);
        }
        rc = mp2650_read_reg(0x2D, &val_buf[MP2650_LAST_REG + 3]);
        if (rc) {
             chg_err("Couldn't  read 0x49 rc = %d\n", rc);
        }
        rc = mp2650_read_reg(0x31, &val_buf[MP2650_LAST_REG + 4]);
        if (rc) {
             chg_err("Couldn't  read 0x48 rc = %d\n", rc);
        }
        rc = mp2650_read_reg(0x33, &val_buf[MP2650_LAST_REG + 5]);
        if (rc) {
             chg_err("Couldn't  read 0x49 rc = %d\n", rc);
        }
        rc = mp2650_read_reg(0x36, &val_buf[MP2650_LAST_REG + 6]);
        if (rc) {
             chg_err("Couldn't  read 0x48 rc = %d\n", rc);
        }
        chg_err("mp2650[2B,2D,31,33,36]: 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n", val_buf[43], val_buf[44], val_buf[45], val_buf[46], val_buf[47]);
    #endif
        chg_err("mp2650[0-0F]: 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n"
        "		mp2650[10-1F]: 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x\n"
        "		mp2650[20-29 48]: 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x48=0x%x\n",
                ...
                val_buf[13], val_buf[14], val_buf[15], val_buf[16], val_buf[17], val_buf[18], val_buf[19], val_buf[20], val_buf[21], val_buf[22], val_buf[23], val_buf[24],
                val_buf[25], val_buf[26], val_buf[27], val_buf[28], val_buf[29], val_buf[30], val_buf[31], val_buf[32], val_buf[33], val_buf[34], val_buf[35], val_buf[36],
                val_buf[37], val_buf[38], val_buf[39], val_buf[40], val_buf[41], val_buf[42]);
    }
    dump_count++;
}
...
static int mp2650_mps_otg_en_gpio_init(struct chip_mp2650 *chip)
{
	...
	if (IS_ERR_OR_NULL(chip->mps_otg_en_sleep)) {
		chg_err("get mps_otg_en_sleep fail\n");
		return -EINVAL;
	}
