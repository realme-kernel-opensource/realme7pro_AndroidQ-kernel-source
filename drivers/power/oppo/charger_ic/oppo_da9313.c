...
static int da9313_config_interface (int RegNum, int val, int MASK)
{
    ...
    //chg_err("ymj read: RegNum: 0x%x val: 0x%x\n", RegNum, da9313_reg);
    ...
    if (ret >= 0) {
        ...
        //chg_err("ymj write: RegNum: 0x%x val: 0x%x ret: %d\n", RegNum, da9313_reg, ret);
    }
    __da9313_read_reg(RegNum, &da9313_reg);
    //chg_err("ymj check: RegNum: 0x%x val: 0x%x\n", RegNum, da9313_reg);
...
static void da9313_parse_dt(struct chip_da9313 *chip)
{
    struct device_node *node = chip->dev->of_node;
    chip->is_ic_en_mode = of_property_read_bool(node, "qcom,ic_en_mode"); //where nONKEY = VCORE puts chip in ACTIVE state, nONKEY = 0V puts chip into POWER_DOWN state.
}

static void da9313_ic_en_mode(struct chip_da9313 *chip)
{
    if (chip->is_ic_en_mode) {
        if (get_project() == OPPO_206B1) {
            //chg_err("da9313 ICEN_MODE,get_PCB_Version:%s\n",get_PCB_Version());
            if ((cmp_pcb("EVT2") >= 0) && (cmp_pcb("PVT") < 0)) {
                da9313_config_interface(REG02_DA9313_ADDRESS, REG02_DA9313_ICEN_MODE_EN, REG02_DA9313_ICEN_MODE_MASK);
                chg_err("da9313 reset and enter ICEN_MODE 1\n");
            } else {
                da9313_config_interface(REG02_DA9313_ADDRESS, REG02_DA9313_ICEN_MODE_DISABLE, REG02_DA9313_ICEN_MODE_MASK);
            }
        } else {
            da9313_config_interface(REG02_DA9313_ADDRESS, REG02_DA9313_ICEN_MODE_EN, REG02_DA9313_ICEN_MODE_MASK);
            chg_err("da9313 reset and enter ICEN_MODE\n");
        }
    }
}
...
#endif
    da9313_parse_dt(divider_ic);
    da9313_hardware_init();
    init_da9313_proc(divider_ic);
    da9313_ic_en_mode(divider_ic);
    return 0;
}
